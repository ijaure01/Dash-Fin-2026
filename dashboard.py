...
st.markdown("""
    ...
    .stApp { background-color: #262C38 !important; color: #FFFFFF !important; }
    [data-testid="stSidebar"] { background-color: #313B45 !important; }
    [data-testid="stWidgetLabel"] p { color: #FFFFFF !important; font-weight: bold !important; }
    div[data-baseweb="select"] > div { background-color: #000000 !important; color: #FFFFFF !important; border: 1px solid #4d5866 !important; }
    div[data-testid="stMetric"] { background-color: #7A8D9C !important; padding: 25px !important; border-radius: 12px !important; box-shadow: 0px 4px 10px rgba(0,0,0,0.3); }
    [data-testid="stMetricValue"] { color: #c9a050 !important; font-size: 48px !important; text-shadow: 1px 1px 2px black; }
    </style>
...
if df is not None:
    ...
    if not df_filtratua.empty:
        ...
        with c_bar:
            fig_top = px.bar(top5, x='Kopurua', y='Deskribapena', orientation='h', color_discrete_sequence=['#738496'])
